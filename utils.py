...
def extract_dir():
  mnistZipLoc = os.path.abspath(os.path.join(os.path.dirname(__file__),os.pardir,"mnist.zip"))
  if not os.path.isfile(mnistZipLoc):
    log.error("MNIST zip not in parent of current directory")
  elif os.path.isdir(MNIST_DIR_LOC):
    log.info("MNIST directory exists")
  else:
    try:
      with zipfile.ZipFile(mnistZipLoc, 'r') as zip_ref:
...
def extract_files():
    ...
    for f in zipped_files:
        ...
        if 'train' in f:
            if 'images' in f:
                unzipped_files['train-imgs'] = fileName
            else:
                unzipped_files['train-labels'] = fileName
        else:
            if 'images' in f:
                unzipped_files['test-imgs'] = fileName
            else:
                unzipped_files['test-labels'] = fileName
        file = (os.path.join(MNIST_DIR_LOC,f))
        unzip_file = (os.path.join(MNIST_DIR_LOC,fileName))
